...
def load_data_splits(data_dir):
    ...
    for i in range(NFOLDS):
        fold_files = [
            {"image": img, "label": lbl}
            for img, lbl in zip(
                train_files["images"][i:15:5], 
                train_files["labels"][i:15:5]
            )
        ]
...
def create_transforms(is_train=True):
    ...
    if is_train:
        ...
        train_transforms = base_transforms + [
            ...
            # Lambda(
            #     lambda x: (print("Augmenting image with shape:", x["image"].shape), x)[1]
            # ),
...
def validate_epoch(
    ...
    writer.add_scalar("Loss/Validation", epoch_loss / len(val_loader), val_step)
...
def main():
    ...
    # Load data
    print("Loading data...")
    folds = load_data_splits("/home/guest/code/data_jsons")
    ...
    for fold_idx, fold_files in enumerate(folds):
        ...
        train_files_fold = [i for fold_i in folds for i in fold_i if i not in val_files]
        ...
        num_epochs = 5  # 100  # Higher number for overfitting
        best_dice = 0
        ...
        for epoch in range(num_epochs):
            ...
            if epoch < 5:  # First 5 epochs
                print(
                    f"  Train Loss: {train_loss:.4f} (Threshold: < 1.0 for good start)"
                )
...
if __name__ == "__main__":
    ...
    test_transforms = base_transforms + [
        ...
        RandCropByPosNegLabeld(
                keys=["image", "label"],
                label_key="label",
                spatial_size=(100, 100, 10),  # heuristic, other values may be best
                pos=1,  # 100% patches must include some label voxel
                neg=0,
                num_samples=3,
            ),
        RandFlipd(keys=["image", "label"], spatial_axis=[0, 1, 2], prob=0.7),
    ...
    trans = Compose(test_transforms)

    out = trans(img)[0]
    print(out)

    print(f"Transformed image shape: {out['image'].shape}")
    # show type and debug by viewing
    print(type(out["image"]))  # Verifica el tipo de dato
    print(type(out["label"]))
