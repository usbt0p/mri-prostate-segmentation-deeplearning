...
def visualize_dicom_slider(image_or_path):
    ...
    Created for exploring the DICOM images in the QIN-PROSTATE-Repeatability dataset.
...
def plot_slices_of_images_with_slider(*images):
    ...
    images = (
        [sitk.ReadImage(path) for path in images]
    ...
    slider = Slider(
        ax_slider, "Slice", 0, images[0].GetSize()[2] - 1, valinit=0, valstep=1
    )
...
def test_label_image_correspondence(zipped):
    ...
    for label_x, image_x in tqdm(
        zipped, total=len(zipped), desc="Checking label-image correspondence"
    ):
        ...
        if label.GetSize() != image.GetSize():
            raise ValueError(
                f"Size mismatch: {label.GetSize()} and {image.GetSize()}\
                            for label\n{label_x}\nand image\n{image_x}"
            )
        ...
        if lround != iround:
            raise ValueError(
                f"Spacing mismatch: {label.GetSpacing()} and {image.GetSpacing()}\
                            for label\n{label_x}\nand image\n{image_x}"
            )
...
def modify_preprocessing_json(preprocessing: str):
    ...
    for key in dataset_json["imagesTr"].keys():
        new_value = dataset_json["imagesTr"][key].replace(
            "Dataset002_picai_", "Dataset002_picai"
        )
        dataset_json["imagesTr"][key] = new_value

    for key in dataset_json["labelsTr"].keys():
        new_value = dataset_json["labelsTr"][key].replace(
            "Dataset002_picai_", "Dataset002_picai"
        )
